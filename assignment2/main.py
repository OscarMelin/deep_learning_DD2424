...
if __name__ == '__main__':
    ...
    if save:
        ...
        plt.show()
        plt.plot(np.arange(len(costs_train))*200,
                 costs_train, 'g', label='training loss')
        plt.plot(np.arange(len(costs_valid))*200,
                 costs_valid, 'r', label='validation loss')
        plt.xlabel("update step")
        plt.ylabel("cost")
        plt.legend()
        plt.show()
        plt.plot(np.arange(len(accs_train))*200,
                 accs_train, 'g', label='training accuracy')
        plt.plot(np.arange(len(accs_valid))*200, accs_valid,
